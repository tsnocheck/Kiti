...
import {ButtonInteraction, EmbedBuilder, type User} from 'discord.js';
  ...
  async run({interaction, client}: { interaction: ButtonInteraction, client: BotClient }): Promise<any> {
    ...
    try {
      ...
      await interaction.reply({
        content: 'Я не смог отправить уведомление пользователю, попробуйте связаться с ним вручную.',
        ephemeral: true
      });
    }
  }
